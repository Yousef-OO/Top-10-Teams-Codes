...
from project import weights_file as weights_path

def evaluate_12ECG_score(label_directory, output_directory, set=''):
    ...
    label_files, output_files = find_challenge_files(label_directory, output_directory, set=set)
...
def find_challenge_files(label_directory, output_directory, set=''):
    label_files = list()
    output_files = list()
    for f in sorted(os.listdir(label_directory)):
        F = os.path.join(label_directory, f) # Full path for label file
        if os.path.isfile(F) and F.lower().endswith('.hea') and not f.lower().startswith('.') and f.startswith(set):
