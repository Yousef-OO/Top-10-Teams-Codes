...
def se_resnet34(num_classes=9):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(SEBasicBlock, [3, 4, 6, 3], num_classes=config.num_classes)
    model.avgpool = nn.AdaptiveAvgPool1d(1)
    return model


def se_resnet50(num_classes=9, pretrained=False):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(SEBottleneck, [3, 4, 6, 3], num_classes=config.num_classes)
    model.avgpool = nn.AdaptiveAvgPool1d(1)
    # if pretrained:
...
def se_resnet101(num_classes=27):
