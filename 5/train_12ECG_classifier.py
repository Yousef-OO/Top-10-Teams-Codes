...
def train_12ECG_classifier(input_directory, output_directory):
    ...
    e1 = tf.layers.conv1d(X, 48, 9, strides=4, padding='valid', activation=tf.nn.relu, use_bias=True)
    e2 = tf.layers.conv1d(e1, 64, 7, strides=3, activation=tf.nn.relu, use_bias=True)
    e3 = tf.layers.conv1d(e2, 80, 5, strides=2, activation=tf.nn.relu, use_bias=True)
    e4 = tf.layers.conv1d(e3, 96, 3, strides=2, activation=tf.nn.relu, use_bias=True)
    e5_0 = tf.layers.max_pooling1d(tf.layers.conv1d(e4, 112, 3, strides=2, activation=tf.nn.relu, use_bias=True), 2, 2)
    ...
    e1 = tf.layers.conv1d(X, 48, 19, strides=4, padding='valid', activation=tf.nn.relu, use_bias=True)
    e2 = tf.layers.conv1d(e1, 64, 15, strides=3, activation=tf.nn.relu, use_bias=True)
    e3 = tf.layers.conv1d(e2, 80, 11, strides=2, activation=tf.nn.relu, use_bias=True)
    e4 = tf.layers.conv1d(e3, 96, 9, strides=2, activation=tf.nn.relu, use_bias=True)
    e5_0 = tf.layers.max_pooling1d(tf.layers.conv1d(e4, 112, 7, strides=2, activation=tf.nn.relu, use_bias=True), 2, 2)
    
    
    print(e5_0.shape)
    
    emb2 = keras.layers.Bidirectional(keras.layers.LSTM(128, recurrent_dropout=0.5, return_sequences=False))(e5_0)
    ...
    for i in range(num_classes):
        e6 = tf.nn.dropout(tf.layers.dense(emb, 100, activation=tf.nn.relu), KEEP_PROB)
    ...
    e5_p = tf.stop_gradient((1. + lambdaa_DG) * emb)
    e5_n = -lambdaa_DG * emb
    e5_d = e5_p + e5_n
