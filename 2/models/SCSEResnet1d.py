...
class SCSELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.spatial_se = nn.Sequential(nn.Conv1d(channel, 1, kernel_size=1,
                                                  stride=1, padding=0),
                                        nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        y = self.channel_excitation(y).view(b, c, 1)
        channel_x = torch.mul(x, y)

        spa_se = self.spatial_se(x)
        spa_x = torch.mul(x, spa_se)

        return torch.add(channel_x, 1, spa_x)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.se = SCSELayer(self.expansion * planes)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
        self.dropout = nn.Dropout(.2)
