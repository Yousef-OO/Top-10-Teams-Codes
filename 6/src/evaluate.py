...
def find_best_thresholds(weights, labels, outputs, classes, normal_class):
    thresholds = np.ones((labels.shape[1]))
    ...
    for i in range(labels.shape[1]):
        ...
        for j in np.arange(0.10, 0.90, 0.01):
            outputs_temp[:, i] = outputs[:, i] > j
